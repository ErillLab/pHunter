...
def GibbsFE(mySeq, GibbsFEdict, moving_avg_wlen=None):
    '''
    Computes the Gibbs Free Energy of a DNA sequence
     The Gibbs Free Energy of a region within the potential promoter sequence is used as a metric to calculate a likelihood in pHunter. Upon comparing this likelihood with others, a log-likelihood may be formed, and scores of potential promoters can be determined.  
     This is the case since Gibbs Free Energy describes the ease at which a particular sequence unzips. 
     In order for Gibbs Free Energy of a region to be determined, sequences must be parsed for dinucleotides and trinucleotides, and the Gibbs Free Energy of these nucleotides must be outputted. In addition, in order to compare the positive and negative likelihood estimates with the region around the -10 sequence, a shared window for averaging energy values must be applied to create a moving average vector. 
    ...
    The GibbsFE function, upon being passed a window length, will also calculate a moving average vector from the energy list. 
     ...
     Inputs:
         ...
         moving_avg_wlen: Int - window size by which the moving average vector will be calculated from the energy vector. 
     
     Returns:
         myVec: List - List containing Gibbs Free Energy values of dinucleotide/trinucleotides of sequence
         myMAVec: List - List containing averages of dinucleotide/trinucleotide values over a certain moving window
    ...
    #Calculates the moving average vector if the parameter moving_avg_wlen is passed into the function
    if moving_avg_wlen:
        
        #Initializes moving average vector
        myMAVec = []
        
        #Ensures the energy vector is not smaller than the averaging window length
        if moving_avg_wlen > len(myVec):
            return myVec
        
        #Computes moving average vector from energy vector by averaging energy values across the window (moving_avg_wlen) and using a step size of 1
        for ener_val in range(len(myVec)-moving_avg_wlen + 1):
    
            #Computes average
            myMAVec.append(sum(myVec[ener_val:ener_val+moving_avg_wlen])/moving_avg_wlen)
